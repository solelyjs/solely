...
```html
<img :src="$data.imageUrl" :alt="$data.imageAlt" /> <a :href="$data.linkUrl">访问链接</a>
```

兼容语法：

```html
<img s-src="$data.imageUrl" s-alt="$data.imageAlt" /> <a s-href="$data.linkUrl">访问链接</a>
```
...
```html
<button :disabled="$data.isLoading">提交</button> <input :checked="$data.isAgreed" />
```

兼容语法：

```html
<button s-disabled="$data.isLoading">提交</button> <input s-checked="$data.isAgreed" />
```
...
```html
<div :class="{ active: $data.isActive, 'has-error': $data.hasError }">内容</div>
<!-- 也支持数组和字符串 -->
...
```html
<div s-class="{ active: $data.isActive, 'has-error': $data.hasError }">内容</div>
<!-- 也支持数组和字符串 -->
...
```html
<div :style="{ color: $data.textColor, fontSize: $data.fontSize + 'px' }">动态样式</div>
```

兼容语法：

```html
<div s-style="{ color: $data.textColor, fontSize: $data.fontSize + 'px' }">动态样式</div>
```
...
1. **嵌套内部切换**：在嵌套路由内部切换时（如 `/nested/child1` ↔ `/nested/child2`），子路由的 `keepAlive` 独立生效

2. **父路由被切换**：当切换到其他父路由（如 `/nested/child1` → `/` 首页）时，**子路由的 `keepAlive` 仍然有效**，子页面状态会被保留

```typescript
// ✅ 场景A：全部缓存（切换到外路由后返回，嵌套状态保留）
{
  path: '/nested',
  tagName: 'nested-layout',
  keepAlive: true,  // 父路由启用
  children: [
    { path: 'child1', tagName: 'child1', keepAlive: true },  // 子路由也启用
    { path: 'child2', tagName: 'child2', keepAlive: true },
  ],
}

// ✅ 场景B：仅子级缓存（父级不缓存，子级缓存仍然有效）
{
  path: '/nested',
  tagName: 'nested-layout',
  // 父路由没有 keepAlive
  children: [
    { path: 'child1', tagName: 'child1', keepAlive: true },  // 子级缓存独立生效
    { path: 'child2', tagName: 'child2', keepAlive: true },
  ],
}
...
// ✅ 场景C：仅布局缓存（子页面始终刷新）
{
  ...
  keepAlive: true,  // 布局缓存
  children: [
    { path: 'child1', tagName: 'child1' },  // 不缓存
    { path: 'child2', tagName: 'child2' },
  ],
}
```
...
- 子路由的 `keepAlive` **独立于父路由**，无论父路由是否启用 `keepAlive`，子路由的缓存都有效
- 使用全局缓存机制，即使父组件被重新创建，子路由的状态也能恢复
